...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // بررسی وجود همه المنت‌های ضروری
    if (!startButton) {
        console.error('دکمه شروع پیدا نشد!');
        return;
    }
    ...
    function toggleFruitSelection(fruit) {
        ...
        if (index === -1) {
            if (selectedFruits.length < 10) {
                ...
                const optionElement = document.querySelector(`.fruit-option[data-fruit="${fruit.name}"]`);
                if (optionElement) {
    ...
    if (existingUser.name && existingUser.group && userNameInput && groupNameInput) {
